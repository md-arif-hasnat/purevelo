...
const products = [
  { name: 'Durum Wheat Vermicelli', image: '/products/vermicelli.svg', note: 'Fine, versatile and made for everyday meals.' },
  { name: 'Premium Penne', image: '/products/penne.svg', note: 'Classic shape for sauces, bakes and salads.' },
  { name: 'Premium Fusilli', image: '/products/fusilli.svg', note: 'Spiral pasta that holds sauces beautifully.' },
  { name: 'Durum Wheat Semolina', image: '/products/semolina.svg', note: 'Premium grain ingredient for culinary applications.' },
];
...
export default function Home() {
  ...
  async function submitContact(e) {
    ...
    try {
      ...
      const data = await res.json(); if (!res.ok) throw new Error(data.error || 'Could not send your enquiry.');
      e.currentTarget.reset(); setStatus('Thank you. Your enquiry has been sent successfully.');
    } catch (err) { setStatus(err.message || 'Could not send your enquiry. Please try again.'); }
    finally { setSending(false); }
  }

  return <>
    <header className="site-header"><div className="nav-shell"><a className="brand" href="#home" aria-label="PureVelo home"><b>PureVelo</b><small>Velora Grain & Foods</small></a><nav className="main-nav"><a href="#home">Home</a><a href="#about">About Us</a><a href="#products">Products</a><a href="#quality">Quality & Manufacturing</a><a href="#b2b">B2B & Distribution</a><a href="#contact">Contact</a></nav><a className="nav-cta" href="#b2b">Become a Distributor</a></div></header>
    <main id="home">
      <section className="hero"><div className="hero-overlay"/><div className="hero-content shell"><div className="hero-copy"><p className="eyebrow">Premium Durum Wheat Foods</p><h1>Pure Ingredients.<br/>Everyday Goodness.</h1><p className="hero-lead">Premium durum wheat vermicelli, pasta and grain-based foods — crafted for dependable quality, modern kitchens and everyday meals.</p><div className="hero-actions"><a className="btn gold" href="#products">Explore Our Products</a><a className="btn outline" href="#b2b">Become a Distributor</a></div><div className="hero-trust"><div><strong>100%</strong><span>Durum Wheat Semolina*</span></div><div><strong>No Maida</strong><span>For applicable products*</span></div><div><strong>Premium</strong><span>Quality Focus</span></div><div><strong>Hygienic</strong><span>Manufacturing</span></div></div></div><div className="hero-visual"><div className="grain-orb orb-one"/><div className="grain-orb orb-two"/><div className="mock-pack pack-red"><span>PureVelo</span><b>PENNE</b><small>Premium Durum Wheat Pasta</small></div><div className="mock-pack pack-green"><span>PureVelo</span><b>FUSILLI</b><small>Premium Durum Wheat Pasta</small></div><div className="pasta-bowl"><span>PASTA</span></div></div></div></section>
    ...
    </main><footer><div className="shell footer-grid"><div className="footer-brand">PureVelo</div><p>Premium Durum Wheat Vermicelli & Pasta</p><p>© 2026 PureVelo · Velora Grain & Foods</p></div></footer>
  </>;
}
